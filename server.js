...
io.on('connection', function(socket) {
  ...
  socket.on('submit_survey', function(data) {
    socket.emit('response', { message: surveyResponse(data) });
...
function surveyResponse(data) {
  // create the response message for submitting survey
...
function luckyNumber() {
  ...
  return `Your lucky number generated by the server is ${Math.floor(Math.random() * 1000 + 1)}`;
}
